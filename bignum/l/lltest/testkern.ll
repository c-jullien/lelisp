;;.TiTre "Tests des primitives de KerN"
...
(defstruct testenv
  (name "")		;; Le nom de la fonction teste'e.
  (flag ())		;; Pour savoir si l'on continue le Test.
  (hist "")		;; L'expression qui provoque l'erreur.
  (depend "")		;; De quoi depend le Test.
)
...
(defvar TESTLENGTH 16)      ;; de la forme 4(n + 1)
...
(defun CheckSubRange (x nd nl)
   ; Verifie l'e'galite' des sous-nombres (RN(x), nd, nl) et (SN(x), nd, nl)
   (tag diff
      (while (neq nl 0)
         (decr nl)
...
(defun TestBnMultiplyDigit (e)
   ...
   (ResetTest 1)
   (ResetTest 2)
   (for (pd 0 1 TESTLENGTH)
      (for (pl 0 1 (- TESTLENGTH pd))
...
(df test s
   (let ( (e (:make)) (n (length s)) )
      ...
      (for (i 0 1 (1- QTL))			    ; Le premier quart est la
         (BnSetDigit NumbProto (+ i 2) (1+ i)) )    ; suite 1, 2, 3, ...
...
(defun dotest (e n)
   ...
   (when (and (tag diff (funcall (vref AllTest (* n 2)) e)) (> (:flag e) 1))
      (exit 0) )
   (print TestCount " tests were performed") )

(defun seetest (n)
   (print n ".	Test de " (vref AllTest (1+ (* n 2)))) )
