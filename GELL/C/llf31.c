...
Ptr Cfloat_to_Lfloat (dflt)
    double dflt; 
{
        register long entier;
        union {long binary ; double floating ; } ftampon ;
        ftampon.floating = dflt;
        entier = ftampon.binary;
        entier = (entier >> 1) | 0x8000000000000000;
        return ((Ptr) entier);
}
    ...
    Ptr ptr;
{
        ...
        ftampon.floating = (float) ((int) ((Fix) ptr));
        return ((Ptr) ((int) ((ftampon.binary >> 1) | 0x80000000)));
}
...
{
        union {int binary ; float floating ; } ftampon ;
        ftampon.binary = (int) ptr << 1;
        return ((Ptr) ((int) ftampon.floating & 0xffff));
}
